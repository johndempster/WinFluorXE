unit nidaqlib;
// ------------------------------------------------------
// National Instruments NIDAQ (traditional) library calls
// ------------------------------------------------------
// 28.05.05
...
type
   ...
   { NIDAQ.DLL procedure  variables }
    ...
    TDIG_Block_PG_Config = function(
                           ...
                           timebase : SmallInt ;
                           reqInterval : Word ;
                           externalGate : SmallInt ) : SmallInt  ; stdcall  ;

    TDIG_Grp_Config  = function(
                       device: SmallInt ;
                       grp : SmallInt ;
                       grpSize : SmallInt ;
                       Port : SmallInt ;
                       Direction : SmallInt ) : SmallInt  ; stdcall  ;

    TDIG_DB_Config = function(
                     device: SmallInt ;
                     grp : SmallInt ;
                     dbMode : SmallInt ;
                     oldDataStop : SmallInt ;
                     partialTransfer : SmallInt ) : SmallInt  ; stdcall  ;

    TDIG_DB_HalfReady = function(
                        device: SmallInt ;
                        grp : SmallInt ;
                        var HalfReady : SmallInt ) : SmallInt  ; stdcall  ;
    ...
    TRTSI_Clear = function(
                  Device : SmallInt ) : SmallInt  ; stdcall  ;
    TRTSI_Conn = function(
                 Device : SmallInt ;
                 SigCode: SmallInt ;
                 TrigLine: SmallInt ;
                 Direction: SmallInt ) : SmallInt  ; stdcall  ;
    TRTSI_DisConn = function(
                    Device : SmallInt ;
                    SigCode: SmallInt ;
                    TrigLine: SmallInt ) : SmallInt  ; stdcall  ;
...
var
   ...
   DIG_Block_PG_Config : TDIG_Block_PG_Config ;
   DIG_Grp_Config : TDIG_Grp_Config ;
   DIG_DB_Config : TDIG_DB_Config ;
   DIG_DB_HalfReady : TDIG_DB_HalfReady ;
   DIG_SCAN_Setup : TDIG_SCAN_Setup ;
   DIG_Prt_Config : TDIG_Prt_Config ;
   DIG_Out_Port : TDIG_Out_Port ;
   DIG_Out_Grp : TDIG_Out_Grp ;
   ...
   RTSI_Clear : TRTSI_Clear ;
   RTSI_Conn : TRTSI_Conn ;
   RTSI_DisConn : TRTSI_DisConn ;
   AI_Configure : TAI_Configure ;

function NIDAQ_LoadLibrary(
         NIDAQ32Hnd : THandle
         ) : Boolean ;

function NIDAQ_LoadProcedure(
...
begin
     ...
     { Load library }
     NIDAQ32Hnd := LoadLibrary( PCHar('NIDAQ32.DLL') ) ;
     if NIDAQ32Hnd <> 0 then begin
       { Get addresses of procedure s used }
        @Init_DA_Brds := NIDAQ_LoadProcedure( NIDAQ32Hnd, 'Init_DA_Brds' ) ;
       { @Init_DA_Brds := GetProcAddress(NIDAQ32Hnd,PChar('Init_DA_Brds')) ;}
        @DAQ_Clear := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DAQ_Clear') ;
        @DAQ_Config := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DAQ_Config') ;
        @DAQ_DB_Config := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DAQ_DB_Config') ;
        @DAQ_Start := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DAQ_Start') ;
        @DAQ_Rate := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DAQ_Rate') ;
        @Lab_ISCAN_Start := NIDAQ_LoadProcedure( NIDAQ32Hnd,'Lab_ISCAN_Start') ;
        @SCAN_Setup := NIDAQ_LoadProcedure( NIDAQ32Hnd,'SCAN_Setup') ;
        @SCAN_Start := NIDAQ_LoadProcedure( NIDAQ32Hnd,'SCAN_Start') ;
        @WFM_Rate := NIDAQ_LoadProcedure( NIDAQ32Hnd,'WFM_Rate') ;
        @WFM_ClockRate := NIDAQ_LoadProcedure( NIDAQ32Hnd,'WFM_ClockRate') ;
        @WFM_Load := NIDAQ_LoadProcedure( NIDAQ32Hnd,'WFM_Load') ;
        @WFM_Group_Control := NIDAQ_LoadProcedure( NIDAQ32Hnd,'WFM_Group_Control') ;
        @WFM_Check := NIDAQ_LoadProcedure( NIDAQ32Hnd,'WFM_Check') ;
        @AI_VScale := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AI_VScale') ;
        @AO_Configure := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AO_Configure') ;
        @AO_Write := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AO_Write') ;
        @AO_Update := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AO_Update') ;
        @AO_VScale := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AO_VScale') ;
        @AO_VWrite := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AO_VWrite') ;
        @DIG_Block_Clear := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Block_Clear') ;
        @DIG_Block_Check := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Block_Check') ;
        @DIG_Block_Out := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Block_Out') ;
        @DIG_Block_PG_Config := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Block_PG_Config') ;
        @DIG_Grp_Config := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Grp_Config') ;
        @DIG_DB_Config := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_DB_Config') ;
        @DIG_DB_HalfReady := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_DB_HalfReady') ;
        @DIG_SCAN_Setup := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_SCAN_Setup') ;
        @DIG_Prt_Config := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Prt_Config') ;
        @DIG_Out_Port := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Out_Port') ;
        @DIG_Out_Grp := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_Out_Grp') ;
        @DIG_In_Port := NIDAQ_LoadProcedure( NIDAQ32Hnd,'DIG_In_Port') ;
        @AI_Read := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AI_Read') ;
        @Get_DAQ_Device_Info := NIDAQ_LoadProcedure( NIDAQ32Hnd,'Get_DAQ_Device_Info') ;        
        @Set_DAQ_Device_Info := NIDAQ_LoadProcedure( NIDAQ32Hnd,'Set_DAQ_Device_Info') ;
        @Select_Signal := NIDAQ_LoadProcedure( NIDAQ32Hnd,'Select_Signal') ;
        @RTSI_Clear := NIDAQ_LoadProcedure( NIDAQ32Hnd,'RTSI_Clear') ;
        @RTSI_Conn := NIDAQ_LoadProcedure( NIDAQ32Hnd,'RTSI_Conn') ;
        @RTSI_DisConn := NIDAQ_LoadProcedure( NIDAQ32Hnd,'RTSI_DisConn') ;
        @AI_Configure := NIDAQ_LoadProcedure( NIDAQ32Hnd,'AI_Configure') ;
        Result := True ;
        end
     else begin
